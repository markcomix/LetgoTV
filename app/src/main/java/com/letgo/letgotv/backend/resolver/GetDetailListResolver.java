...
/**
 * Make the backend call to get the Similar TV Shows Feed List
 */
public class GetDetailListResolver extends BaseResolver {
     ...
     * @return
     */
    public Observable<ResultListResponse> makeCall(final int page, final int showId) {
        ...
        return Observable.create(new Observable.OnSubscribe<ResultListResponse>() {
                 ...
                 public void call(final Subscriber<? super ResultListResponse> subscriber) {
                     ...
                     if (!useMockData) {
                         ...
                         getDataFromBackend(subscriber, page, showId);
    ...
    private void getMockData(Subscriber<? super ResultListResponse> subscriber) {
        ...
        list.add(new ResultEntity(1, "Movie 1", "Bla bla bla", "", 7.55f));
        list.add(new ResultEntity(2, "Movie 2", "Bla bla bla", "", 9.5f));
        list.add(new ResultEntity(3, "Movie 3", "Bla bla bla", "", 10f));
     ...
     * @param page
     */
    private void getDataFromBackend(final Subscriber<? super ResultListResponse> subscriber, int page, int showId) {
        ...
        if (!CheckConnection.getInstance().isOnline()){
            ...
            //Make the call
            client.getDetail(String.valueOf(showId),
                    AppConstants.Backend.API_KEY,
                    Locale.getDefault().getLanguage(),
                    page).enqueue(new Callback<ResultListResponse>() {
